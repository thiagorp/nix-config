...
# Run it as:
# $ ./bin/update_vscode_extensions.sh > home/thiago/vscode/extensions.nix
...
function get_vsixpkg() {
    ...
    HTTP_CODE=$(curl --silent --show-error --write-out "%{http_code}" --retry 3 -X GET -o "$EXTTMP/$N.zip" "$URL")

    if [[ ${HTTP_CODE} -ne "200" ]]; then
      >&2 echo "$N failed with ${HTTP_CODE}"
      return 0
    fi
